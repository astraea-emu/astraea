...
inline constexpr std::size_t kPm4UserConfigRegisterWindowDwords = 0x400;
...
struct Pm4SetUconfigRegLowerError {
    Pm4SetUconfigRegLowerErrorCode code =
        Pm4SetUconfigRegLowerErrorCode::malformed_set_uconfig_reg;
    std::size_t word_offset = 0;
    std::uint32_t raw_offset_control_word = 0;
    std::size_t value_count = 0;

    auto operator<=>(const Pm4SetUconfigRegLowerError&) const = default;
};
...
// Generic AMD SET_UCONFIG_REG lowering for the 0xc000..0xc3ff register window.
// This bounded form accepts only an offset/control word whose upper 16 bits
// are zero. Indexed/control variants remain unsupported.
