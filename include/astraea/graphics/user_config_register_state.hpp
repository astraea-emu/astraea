...
struct UserConfigRegisterState {
    std::array<std::uint32_t, kUserConfigRegisterStateDwords> values{};
    std::bitset<kUserConfigRegisterStateDwords> initialized{};
...
struct UserConfigRegisterWriteEffect {
    ...
    auto operator<=>(const UserConfigRegisterWriteEffect&) const = default;
};
...
struct UserConfigRegisterApplyError {
    UserConfigRegisterApplyErrorCode code =
        UserConfigRegisterApplyErrorCode::unsupported_operation;
    std::uint16_t start_offset = 0;
    std::size_t value_count = 0;
    std::optional<GraphicsIrUnsupportedReason> unsupported_reason;

    auto operator<=>(const UserConfigRegisterApplyError&) const = default;
};
