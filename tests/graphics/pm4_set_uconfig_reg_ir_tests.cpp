...
const astraea::graphics::GraphicsIrUserConfigRegisterWriteRange&
require_uconfig_range(
...
TEST_CASE(
    "SET_UCONFIG_REG lowers primitive type exactly",
    ...
    const auto& operation =
        require_uconfig_range(emission);
    REQUIRE(operation.start_offset == 0x0242U);
    REQUIRE(
        operation.values ==
...
TEST_CASE(
    "SET_UCONFIG_REG preserves consecutive generic values",
    "[graphics][pm4][set-uconfig-reg]") {
    const auto emission =
        lower_one({
            make_type3_header(
    ...
    const auto& operation =
        require_uconfig_range(emission);
    REQUIRE(operation.start_offset == 0x025bU);
    REQUIRE(
        operation.values ==
...
TEST_CASE(
    "non-SET_UCONFIG_REG opcode remains unsupported Graphics IR",
...
TEST_CASE(
    "SET_UCONFIG_REG rejects malformed and controlled forms",
...
TEST_CASE(
    "SET_UCONFIG_REG enforces the 0x400-dword window",
    "[graphics][pm4][set-uconfig-reg][boundary]") {
...
TEST_CASE(
    "SET_UCONFIG_REG semantic equality ignores packet provenance",
