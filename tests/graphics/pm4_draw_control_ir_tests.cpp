...
TEST_CASE(
    "NUM_INSTANCES lowers one exact payload value",
    "[graphics][pm4][draw-control]") {
    const auto emission =
        lower_one({
            make_type3_header(
...
TEST_CASE(
    "DRAW_INDEX_AUTO preserves count and raw initiator",
    "[graphics][pm4][draw-control]") {
    ...
    const auto& draw =
        std::get<
            astraea::graphics::GraphicsIrDrawIndexAuto>(
            emission.operation);
...
TEST_CASE(
    ...
    SECTION("NUM_INSTANCES extra payload") {
        auto frame =
            frame_one({
                make_type3_header(
                    astraea::graphics::
