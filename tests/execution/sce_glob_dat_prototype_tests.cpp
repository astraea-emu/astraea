...
constexpr std::size_t kPointerOffset = 0x80;
...
const std::string kRawImportName =
    "GLOBDATNID1#library-g#module-g";
...
struct PrototypeFixture {
    ...
    std::uint64_t page = 0;
    std::uint64_t pointer_address = 0;
};
...
TEST_CASE(
    "owned PS5 SCE-profile ELF resolves GLOB_DAT import and exits through HLE",
    "[execution][sce-prototype][glob-dat][m4]") {
    ...
    REQUIRE(built->general_relocations.rela.has_value());
    REQUIRE(
        built->general_relocations.rela->kind ==
        astraea::loader::
            RelocationTableKind::rela);
    REQUIRE_FALSE(built->plt_relocations.has_value());
    ...
    auto relocation =
        astraea::loader::
            parse_dynamic_relocation(
                *built->general_relocations.rela,
                0,
                *built->dynamic_symbols,
                view.value());
    ...
    REQUIRE(pointer_bytes == patch->bytes);

    std::uint64_t pointer_destination = 0;
    for (std::size_t index = 0;
         index < pointer_bytes.size();
         ++index) {
        pointer_destination |=
            static_cast<std::uint64_t>(
                std::to_integer<
