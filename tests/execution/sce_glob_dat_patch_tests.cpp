...
TEST_CASE(
    "x86-64 GLOB_DAT patch encodes exact synthetic gate address",
    "[execution][sce-glob-dat]") {
...
TEST_CASE(
    "x86-64 GLOB_DAT patch requires general RELA source",
    "[execution][sce-glob-dat]") {
    const auto gates = make_gate_region();
...
TEST_CASE(
    "x86-64 GLOB_DAT patch rejects other relocation types",
    "[execution][sce-glob-dat]") {
    const auto gates = make_gate_region();
    ...
    REQUIRE(result.error().relocation_type == 7);
}

TEST_CASE(
    "x86-64 GLOB_DAT patch requires an explicitly bound gate slot",
    "[execution][sce-glob-dat]") {
    const auto gates = make_gate_region();
...
TEST_CASE(
    "x86-64 GLOB_DAT gate function must match resolved import",
    "[execution][sce-glob-dat]") {
    const auto gates = make_gate_region();
...
TEST_CASE(
    "GLOB_DAT raw addend remains evidence and does not alter S bytes",
    "[execution][sce-glob-dat]") {
    const auto gates = make_gate_region();
